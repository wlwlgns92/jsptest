...
<body>
	...
	<%
		int no = WorkDao.getWorkDao().getno();
		if( no == -1){ no =1; }
		else{ no++; } 
	%>
	...
	<h3>작업공정등록</h3>
		작업지시번호<input type="text" value="<%=no%>" name="no">예) 20190001 <br>
		...
		합지공정 <input type="radio" id="stick1" name="stick" value="완료">완료 <input type="radio" id="stick2" name="stick" value="작업중"> 작업중	<br>
		<div id="stickmsg"></div>
		...
		최종작업일자 <input type="text" id="fdate" name="fdate"> 예) 20190101 <br>
		<div id="fdatemsg"></div>
		최종작업시간 <input type="text" id="ftime" name="ftime"> 예) 1300
		<div id="ftimemsg"></div>
