...
public class WorkDao extends DB{
	...
	public ArrayList<WorkDto> worklist() {
		...
		try {
			...
			rs = ps.executeQuery();
			while(rs.next()) {
				WorkDto workDto = new WorkDto(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9));
